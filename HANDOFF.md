...
- `crm_collaborations.requester_share` (int, default 50). Owner share = 100 −
  it. `crm_request_collaboration(..., p_requester_share int default 50)` (old
  4-arg version dropped) accepts **50–90** only (default 50/50; the requester
  may propose MORE for themselves) and stamps `agreement_version = 'v2'`. The
  admin review and owner notifications state the split. Proven rolled back: 40
  refused, 60 stored as "60 v2", admin note "sara zangeneh 40% / mehdi sarraf
  60%".
- Popup: "Commission split" card (presets 50/50, 40/60, 30/70, a 50–90 slider in
  steps of 5, a two-colour bar "<owner> X% | You Y%"); agreement term 3 is
  generated by `collaborationTerms({version, ownerName, requesterName,
  requesterShare})` (v1 rows keep the old "decided by management" wording).
  Board rows show "split 50/50"; the details dialog and Approve/Accept confirms
  quote the split.
...
- `TEMPERATURE_OPTIONS` (one list, used by Leads + Contacts cells, filters,
  drawers, export) = Cold (blue #0086c0), Pending (#fdab3d), Warm (#00c875), **Void** (red #e2445c). Colours were changed twice on request the same day; Cold went grey then back to BLUE on 09-20, in both Lead status and Contact status (one shared option list). CHECK constraints
