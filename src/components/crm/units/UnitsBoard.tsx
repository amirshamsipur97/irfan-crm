...
export function UnitsBoard({
  ...
  const requestDeleteGroup = (group: (typeof groups)[number]) => {
    const count = localUnits.filter((r) => r.group_id === group.id).length;
    if (count) {
      setToast({
        message: `"${group.name}" still holds ${count} unit${count === 1 ? "" : "s"} — move or delete them first.`,
        tone: "alert",
      });
      return;
    }
      ...
      {deleteGroupPrompt && (
        <ConfirmDialog
          ...
          onConfirm={async () => {
            ...
            const result = await deleteUnitGroup(g.id);
            if (result.error) {
              setLocalGroups(prev);
              setToast({ message: result.error, tone: "alert" });
            } else {
