...
export function ActivitiesBoard({
  ...
  const requestDeleteGroup = (group: (typeof groups)[number]) => {
    const count = localActivities.filter((r) => r.group_id === group.id).length;
    if (count) {
      setToast({
        message: `"${group.name}" still holds ${count} activity${count === 1 ? "" : "s"} — move or delete them first.`,
        tone: "alert",
      });
      return;
    }
  ...
  const filterDims: QuickFilterDim<CrmActivityItem>[] = [
    ...
    { key: "type", label: "Activity Type", get: (r) => r.activity_type },
  ...
  const handleAddActivity = async (groupId: string, name: string) => {
    ...
    const created = await addActivity(groupId, name);
    if (created.error || !created.row) {
      setLocalActivities((prev) => prev.filter((r) => r.id !== tempId));
    ...
    <Surface>
      <div ref={rootRef} className="flex h-full flex-col">
        <div className="board-anim">
          <BoardHeader
            quickFilters={{ dims: filterDims, rows: localActivities, state: qf.state, onToggle: qf.toggle, onClear: qf.clear, visible: sortedRows.length, noun: "activities" }}
            profile={profile}
            title="Activities"
            tabs={["Main table"]}
      ...
      {deleteGroupPrompt && (
        <ConfirmDialog
          ...
          onConfirm={async () => {
            ...
            const result = await deleteActivityGroup(g.id);
            if (result.error) {
              setLocalGroups(prev);
              setToast({ message: result.error, tone: "alert" });
            } else {
