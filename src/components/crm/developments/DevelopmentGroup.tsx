...
import type { CrmDevelopment, CrmDevelopmentGroup, CrmUnit, CrmUser } from "@/lib/types";
import {
  BatteryBar,
  Checkbox,
  InlineEdit,
...
export function DevelopmentGroup({
  group,
  developments,
  units,
  users,
  ...
  const statusSegments = DEVELOPMENT_STATUSES.map((s) => ({
    color: s.color,
    count: developments.filter((d) => d.status === s.key).length,
  })).filter((s) => s.count > 0);

  const unitCount = (developmentId: string) =>
    units.filter((u) => u.development_id === developmentId).length;
                ...
                switch (col.key) {
                  case "owner":
                    return (
                      <span key={col.key} className={`${cellBorder} block bg-white`} style={w}>
                        <OwnerCell
                          owner={users.find((u) => u.id === dev.owner_id)}
                          users={users}
                          onSelect={(ownerId) => onPatch(dev.id, { owner_id: ownerId })}
                        />
                      </span>
                    );
                  case "developer":
                    return (
                      <span key={col.key} className={`${cellBorder} block bg-white`} style={w}>
                        <ConnectPicker
                          ...
                          kind="account"
                          onPick={(name) => onPatch(dev.id, { developer_name: name })}
                          onClear={() => onPatch(dev.id, { developer_name: null })}
                          onCreate={(name) => onCreateDeveloper(dev.id, name)}
                        />
                      </span>
                    );
                  case "status":
                    return (
                      <span key={col.key} className={`${cellBorder} block bg-white`} style={w}>
                        <OptionCell
                          ...
                          onSelect={(next) => onPatch(dev.id, { status: next })}
                        />
                      </span>
                    );
                  case "location":
                    return (
                      <span key={col.key} className={`${cellBorder} block bg-white`} style={w}>
                        <CenterEditCell
                          value={dev.location}
                          onSave={(next) => onPatch(dev.id, { location: next })}
                        />
                      </span>
                    );
                  case "completion":
                  ...
                  case "units":
                    return (
                      ...
                      >
                        {unitCount(dev.id) > 0 && (
                          <span className="rounded-[12px] bg-cyan-soft px-[10px] py-[2px] font-sans text-[13px] leading-[18px] text-teal-deep">
                            {unitCount(dev.id)}
                          </span>
                        )}
                      </span>
                    ...
                    return (
                      <span key={col.key} className={`${cellBorder} block bg-white`} style={w}>
                        <TextCell
                          ...
                          onSave={(next) => onPatch(dev.id, { description: next || null })}
                        />
                      </span>
                    );
                  default:
